...
{
    // Reset movement each frame; AI re-applies below
    target.move(0);
    ...
    // Always chase the ball horizontally
    if (std::abs(dx) > 40.0f) {
    ...
    // Decision timer for jump / kick (every ~130-230 ms)
    decisionTimer -= 1.0f;
    if (decisionTimer <= 0.0f) {
        ...
        // Jump when ball is above us and close enough
        ...
        // Kick when ball is near our feet
